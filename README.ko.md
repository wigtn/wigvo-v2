...
<br />
<em>실시간 통화 인터페이스 — 양방향 자막, 통화 상태, 채팅 기록</em>
</div>
...
<div align="center">
<img src="docs/paper/figures/figure2_pipeline.png" alt="WIGVO 파이프라인 아키텍처" width="100%" />
</div>
...
AudioRouter (얇은 위임자)
    │
    ├── VoiceToVoicePipeline  ← Echo Gate + Silence Injection + 전체 오디오 경로
    ├── TextToVoicePipeline   ← Chat API 번역 (Session B) + per-response instruction (Session A)
...
> *Figure 1: WIGVO 시스템 아키텍처 — 이중 세션 릴레이 서버가 웹/모바일 클라이언트, OpenAI Realtime API, Twilio PSTN 전화를 Supabase를 통해 연결*

두 개의 독립적인 OpenAI Realtime 세션이 동시에 운영됩니다 — **Session A** (User → 수신자)와 **Session B** (수신자 → User). 단일 세션은 양방향 대화에서 번역 방향이 혼동되기 때문입니다.

---

## 핵심 기술

### 에코 방지 — Silence Injection

Session A TTS 출력이 전화망을 통해 Session B로 에코되면 무한 번역 루프가 발생합니다. 하드웨어 AEC 없이 소프트웨어만으로 해결:

- **Silence Injection**: TTS 재생 중 수신 오디오를 mu-law 무음(`0xFF`)으로 대체
- **Dynamic Cooldown**: TTS 길이에 비례하는 대기 시간 + 0.3s 에코 왕복 마진
- **Energy Breakthrough**: 에코 윈도우 중 고에너지 발화(500+ RMS) 감지 시 즉시 게이트 해제
- **Post-Echo Settling**: 에코 윈도우 종료 후 AGC 안정화 대기
...
OpenAI Server VAD는 PSTN 잡음(RMS 50-200)에서 발화 종료를 감지하지 못합니다 (15-72초 지연). WIGVO는 2단계 로컬 VAD로 해결:

- **Stage 1**: RMS 에너지 게이트 — 150 RMS 미만은 무음 처리 (CPU 절약)
- **Stage 2**: Silero 신경망 VAD — 히스테리시스 상태 머신으로 정확한 발화 경계 감지
- **결과**: 발화 종료 감지 15-72초 → **480ms**
...
│   │   │   │   ├── text_to_voice.py  # T2V (Chat API 번역 + per-response instruction)
│   │   │   │   └── full_agent.py     # Agent (function calling, 자율)
│   │   │   ├── chat_translator.py    # Chat API 번역기 (GPT-4o-mini, T2V/Agent Session B)
│   │   │   ├── audio_router.py      # 얇은 위임자 → 파이프라인 선택
│   │   │   ├── audio_utils.py       # 공유 mu-law 오디오 유틸리티
│   │   │   ├── sessions/            # OpenAI Realtime 세션 핸들러
...
Google Cloud Run 프로덕션 환경의 실제 통화 데이터. `scripts/eval/`로 Supabase에서 수집.
...
| 구간 | P50 | P95 | Mean | N |
|------|----:|----:|-----:|--:|
| **Session A** (User→수신자) | 557ms | 1156ms | 617ms | 814턴 |
| **Session B E2E** (수신자→User) | 2868ms | 15482ms | 3997ms | 744턴 |
| Session B STT만 | 2675ms | 15547ms | 3868ms | 744턴 |
| Session B 번역만 | 104ms | 1934ms | 535ms | 585턴 |
| 첫 메시지 레이턴시 | 1215ms | 6890ms | 2081ms | 162건 |
| 발화 길이 | 1640ms | 5073ms | 2168ms | — |

### 발화 길이별 레이턴시

| 문자 수 | N | P50 | P95 | Mean |
|---------|--:|----:|----:|-----:|
| ≤30자 | 565 | 2502ms | 6511ms | 3119ms |
| 31–60자 | 132 | 4713ms | 15692ms | 5860ms |
| 61–100자 | 32 | 7596ms | 18402ms | 8922ms |
| 100+자 | 15 | 11100ms | 18716ms | 10185ms |
...
*비용: OpenAI Realtime API (오디오 입력 $0.06/1K, 오디오 출력 $0.24/1K) + GPT-4o-mini Chat API (T2V/Agent 번역)*
