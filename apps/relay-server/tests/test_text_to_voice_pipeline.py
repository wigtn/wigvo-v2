...
핵심 검증 사항:
  ...
  - Echo Gate/EchoDetector 비초기화
  - Session A TTS → Twilio 직접 전달 (echo recording 없음)
...
class TestTextToVoicePipelineCreation:
    ...
    def test_no_echo_detector(self):
        """TextToVoice는 EchoDetector를 초기화하지 않는다."""
        router = _make_router()
        assert not hasattr(router._pipeline, "_echo_detector")

    def test_no_echo_suppression(self):
        """TextToVoice는 echo suppression 상태를 갖지 않는다."""
        router = _make_router()
        assert not hasattr(router._pipeline, "_echo_suppressed")
...
class TestTextToVoiceAudioHandling:
    ...
    async def test_twilio_audio_passes_through(self):
        """Twilio 수신자 오디오는 Session B에 전달된다."""
        router = _make_router()
...
class TestTextToVoiceSessionACallbacks:
    ...
    @pytest.mark.asyncio
    async def test_tts_sent_to_twilio_without_echo_recording(self):
        """TTS 오디오가 에코 기록 없이 Twilio에 직접 전달된다."""
        router = _make_router()
        router.interrupt = MagicMock()
        router.interrupt.is_recipient_speaking = False
