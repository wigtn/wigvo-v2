...
const SEARCH_TOOL: OpenAI.Chat.ChatCompletionTool = {
  ...
  function: {
    name: 'search_place',
    description:
      '네이버 지역검색으로 가게/장소를 검색합니다. 가게 이름, 전화번호, 주소를 찾을 수 있습니다. 사용자가 상호명, 지역, 장소, 업종 중 하나라도 언급하면 반드시 이 도구로 검색하세요.',
    parameters: {
      type: 'object',
      properties: {
        query: {
...
- 사용자가 상호명, 지역, 장소, 업종을 언급하면 반드시 검색하세요.
- 상호명을 정확히 모르더라도 지역+업종으로 검색 가능합니다.
- 검색 결과에서 전화번호를 확보하세요.
