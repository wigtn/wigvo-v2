...
We present \textbf{WIGVO}, a server-side relay that bridges a web client and a standard PSTN phone number through two concurrent LLM-backed streaming sessions. The caller speaks (or types) via a browser; the callee answers on an ordinary phone---no app installation, carrier integration, or special hardware is required on either end. WIGVO translates bidirectionally in real time, supporting voice-to-voice (V2V), text-to-voice (T2V), and full-agent modes, with an additional voice-to-text (VTT) accessibility mode implemented but not evaluated in this study.
...
\paragraph{Session~B latency and full-duplex models.}
Session~B latency (${\sim}$2\,s median) introduces noticeable delay, primarily due to streaming ASR. As production-ready full-duplex speech models mature~\citep{defossez2024moshi, labiausse2025hibiki}, this bottleneck may be reduced by replacing the STT stage with an end-to-end audio model. The PSTN relay layer, echo gate, and VAD architecture would remain necessary regardless, as they address network-level constraints orthogonal to model design.
