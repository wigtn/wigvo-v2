...
\begin{figure}[t]
  ...
  \caption{WIGVO architecture. The relay server manages Session~A (caller$\to$callee) and Session~B (callee$\to$caller) as independent Realtime API connections, with a telephony gateway for PSTN audio. The dashed line shows the echo path that Echo Gate suppresses via silence injection.}
