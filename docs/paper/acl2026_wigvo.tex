...
\begin{enumerate}
    ...
    \item A dynamic cooldown of 0.3\,s (0.5\,s for T2V mode) accounts for PSTN jitter before resuming normal forwarding.
\end{enumerate}
...
A 15-pattern blocklist intercepts Korean broadcast-style hallucinations from Whisper on low-energy input; 5 outputs were blocked across 37 instrumented calls.

\section{Evaluation}
\label{sec:evaluation}

We evaluated WIGVO on 96 PSTN calls (37 fully instrumented) for Korean$\leftrightarrow$English translation.
...
An English-speaking user calls a Korean restaurant. Session~A translates caller utterances into Korean TTS for the callee; Session~B returns Korean responses as English audio and captions. Across 37 instrumented calls, Session~A achieved 562\,ms median latency (P95: 1{,}023\,ms) and Session~B 1{,}994\,ms median (P95: 5{,}390\,ms; $r{=}0.529$ with utterance length, $p{<}0.001$).
...
This work was conducted independently by the WIGTN crew at SoundMind Inc. We thank the reviewers for their feedback. WIGVO uses OpenAI's Realtime API and Twilio Media Streams. The relay server is implemented in Python with FastAPI.
