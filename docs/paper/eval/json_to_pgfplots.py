...
def convert(input_path: Path, label: str = "") -> None:
    with open(input_path) as f:
        data = json.load(f)

    _DATA_DIR.mkdir(parents=True, exist_ok=True)

    # label이 있으면 파일명에 suffix 추가 (e.g. "_feb23_26")
    ...
    for fname, bins in [(f"figure3_sa_hist{suffix}.csv", sa_bins), (f"figure3_sb_hist{suffix}.csv", sb_bins)]:
    ...
    path = _DATA_DIR / f"figure4_scatter{suffix}.csv"
    with open(path, "w") as f:
    ...
    path = _DATA_DIR / f"figure4_meta{suffix}.tex"
    with open(path, "w") as f:
        label_comment = f" (label={label})" if label else " (all data)"
        f.write(f"% Auto-generated by json_to_pgfplots.py -- do not edit{label_comment}\n")
...
def main() -> None:
    ...
    parser.add_argument("--label", default="", help="Output file suffix/label (e.g. 'feb23_26')")
    ...
    label_str = f" [{args.label}]" if args.label else " [all]"
    print(f"Input:  {input_path}")
    print(f"Label:  {label_str}")
    print(f"Output: {_DATA_DIR}/\n")
    convert(input_path, label=args.label)
