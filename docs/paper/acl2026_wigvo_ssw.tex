...
Streaming speech-to-speech translation has advanced rapidly with Realtime LLM APIs~\citep{openai_realtime_api_2024}, but existing work focuses on WebRTC and VoIP settings where wideband audio and client-side acoustic echo cancellation (AEC) are available~\citep{seamless_streaming_2023,whisper_radford_2023}. The Public Switched Telephone Network (PSTN), however, remains the primary inbound telephony interface for local businesses, hospitals, and government offices~\citep{itu_pstn_overview_2020}. PSTN encodes audio via G.711 $\mu$-law at 8\,kHz~\citep{itu_g711_1988}, introduces end-to-end delays of 80--600\,ms~\citep{itu_g114_2003}, and provides no client-side AEC. For foreign residents, people with speech anxiety~\citep{kim_oh_telephone_anxiety_2023}, or users with hearing impairments, making such calls remains a significant barrier.
...
Whisper occasionally generates broadcast-style hallucinations on low-energy PSTN input---for example, Korean news sign-off phrases such as ``MBC News'' or ``Thank you for watching.'' A 15-pattern blocklist intercepts these outputs before they reach the translation stage; 5 hallucinated outputs were blocked across 37 instrumented calls.
...
Across the 37 instrumented calls (45.5 minutes total), WIGVO consumed 638{,}035 tokens at a total cost of USD\,19.05, yielding \textbf{USD\,0.42/min} (USD\,0.50/call). These costs reflect dual-session Realtime API pricing and remain an order of magnitude cheaper than human relay services (USD\,1--3/min).
...
Per-minute cost (USD\,0.42) is driven by Realtime API audio token pricing; self-hosted STT alternatives could reduce this. A formal user study (e.g., SUS assessment) with target populations has not yet been conducted and is planned as future work.
...
This work was conducted independently by the WIGTN crew at SoundMind Inc. We thank the anonymous reviewers for their feedback. WIGVO uses OpenAI's Realtime API and Twilio Media Streams. The relay server is implemented in Python with FastAPI.
