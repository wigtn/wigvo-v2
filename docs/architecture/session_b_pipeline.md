...
Callee Phone ──PSTN──→ Twilio Media Stream ──[g711_ulaw 8kHz]──→ Relay Server
                      ...
                      OpenAI Session B (input: g711_ulaw 8kHz)
                           ...
                           │
                           ├─── V2V 경로 ─────────────────────────────
                           │    ├─ Translation: Realtime API 내부 번역
                           │    └─ TTS: pcm16 24kHz 출력
                           │
                           └─── T2V/Agent 경로 ────────────────────────
                                ├─ stt_text → ChatTranslator (Chat API, gpt-4o-mini)
                                └─ text-only 출력 (TTS 없음)
                                  │
                                  ▼
                      Relay Server ──[pcm16 24kHz, WebSocket 직접]──→ User App (V2V: 스피커 재생)
...
self.session_b = RealtimeSession(
    ...
    config=SessionConfig(
        ...
        input_audio_transcription={"model": stt_model, "language": target_language},
    ),
)
```
...
Twilio ──[g711_ulaw 20ms]──→ handle_twilio_audio()
                                   ...
                                   │        rms = ulaw_rms(audio)
                                   │        if rms > 500 (echo_energy_threshold):
...
#       if rms > echo_energy_threshold_rms (500):  → gate break + 원본 반환
...
이유: 에코 노이즈가 speech로 오감지되는 것을 방지. Echo Window 종료 후에도 **Post-Echo Settling (2.0s)** 동안 VAD 처리를 계속 억제한다 (`is_suppressing = True`). AGC(자동 이득 제어) 복원 시 마이크 게인이 급등하면서 발생하는 노이즈가 speech로 오감지되는 것을 방지한다.
                              ...
                              session_b.notify_speech_started()
                                   ...
                                   └─ on_recipient_speech_started 콜백
                                        → _on_recipient_started() [Pipeline]
                                            ...
                                            ├─ [V2V] 이미 전송됨 → interrupt.on_recipient_speech_started()
                                            └─ [T2V] TTS 생성 중(is_generating) → interrupt 차단 (메시지 완전 전달 우선)
```
...
LocalVAD SPEAKING→SILENCE ──→ _on_local_vad_speech_end()
                              ...
                              session_b.notify_speech_stopped(peak_rms)
                                   │
                                   ├─ 최소 발화 길이 필터 (< 250ms → 무시 + buffer clear)
                                   ├─ Peak RMS 필터 (< 300 RMS → 노이즈/잔향 → 무시 + buffer clear)
...
### 5. STT + Translation Output

#### V2V 모드 (Realtime API 통합 처리)
...
OpenAI Session B
    ...
    │       ├─ Whisper 할루시네이션 필터 (15 패턴 blocklist, V2V whisper-1 전용)
    │       ├─ STT 레이턴시 임시 저장 (_pending_stt_ms)
    ...
    └─ [response.audio_transcript.done] ──→ 번역 완료
            ...
            ├─ STT latency와 E2E를 동시에 기록 (리스트 인덱스 정합성 보장)
            ├─ processing latency 기록 (speech_stopped → 번역 완료)
            ├─ transcript_bilingual 저장 (원문 + 번역)
            ├─ context_manager.add_turn("recipient", text)
            └─ App에 TRANSLATION_STATE: "caption_done" 전송
```

#### T2V/Agent 모드 (Realtime STT + Chat API 번역)

T2V/Agent에서는 Realtime API의 번역 대신 Chat API를 사용한다:

```
OpenAI Session B (modalities=['text'], context_prune_keep=0)
    │
    ├─ [input_audio_transcription.completed] ──→ Stage 1 자막 (원문 STT)
    │   "안녕하세요, 예약하고 싶은데요" (targetLanguage)
    │       ├─ Whisper 할루시네이션 필터
    │       ├─ App에 CAPTION_ORIGINAL 전송
    │       └─ _stt_text 저장 + _stt_ready_event.set()
    │
    ├─ [response.done] ──→ response 완료 시 Chat API 번역 트리거
    │       ├─ _stt_ready_event 대기 (최대 10s)
    │       ├─ ChatTranslator.translate(stt_text)
    │       │   └─ Chat API (gpt-4o-mini, temperature=0)
    │       │       System: "Translate only, no extra words"
    │       │       Context: 2턴 슬라이딩 윈도우
    │       ├─ translated_text → App에 CAPTION_TRANSLATED 전송
    │       ├─ transcript_bilingual 저장 (원문 + 번역)
    │       ├─ context_manager.add_turn("recipient", text)
    │       ├─ Chat 토큰 비용 기록 (CostTokens.chat_input/chat_output)
    │       └─ App에 TRANSLATION_STATE: "caption_done" 전송
    │
    └─ (audio 이벤트 없음 — modalities=['text']이므로 TTS 미생성)
```

**context_prune_keep=0의 역할**: Realtime API 세션의 컨텍스트 아이템을 전부 삭제하여 Realtime이 자체적으로 번역을 시도하는 것을 방지한다. 번역 컨텍스트는 ChatTranslator가 별도로 관리한다.
...
`speech_duration < 250ms`인 segment는 노이즈로 간주하고 무시 + buffer clear.
한국어 짧은 단어("여보세요?" ~290ms)는 통과, 순수 노이즈(<200ms)는 차단.

### Peak RMS Quality Filter

`peak_rms < 300` (session_b_min_peak_rms)인 speech는 에너지가 약하므로 노이즈/잔향으로 간주하고 무시.
조용한 PSTN 발화(200-500 RMS)도 통과하도록 설정.
...
STT latency는 `_handle_stt_transcript()`에서 계산되고, E2E latency는 `_save_transcript_and_notify()`에서 계산된다.
두 이벤트의 발생 시점이 다르므로 `_pending_stt_ms` 패턴으로 **동시에 리스트에 기록**하여 인덱스 정합성을 보장한다:

```python
# STT handler: 임시 저장만
self._pending_stt_ms = stt_ms

# E2E handler: STT와 E2E를 동시에 append
call_metrics.session_b_e2e_latencies_ms.append(e2e_ms)
call_metrics.session_b_stt_latencies_ms.append(self._pending_stt_ms or e2e_ms)
self._pending_stt_ms = 0.0
```

`[unclear]` 필터에서 E2E 기록을 건너뛸 때 `_pending_stt_ms`도 함께 폐기하여 리스트 길이 불일치를 방지한다.

### 추가 메트릭

| 메트릭 | 구간 | 용도 |
|--------|------|------|
| `session_b_speech_durations_ms` | speech_started → speech_stopped | 수신자 발화 길이 (노이즈 필터 통과 후) |
| `session_b_processing_latencies_ms` | speech_stopped → 번역 완료 | 처리 지연 (STT와 독립적) |
| `session_b_stt_after_stop_ms` | speech_stopped → STT 완료 | STT가 speech 종료 후 얼마나 더 걸렸는지 |
...
## T2V/Agent 모드: STT + Chat API 분리 아키텍처

Text-to-Voice/FullAgent 파이프라인에서 Session B는 **Realtime API STT + Chat API 번역** 분리 아키텍처를 사용한다.

### 변경 동기

Realtime API는 "생성(generative)" 모델로, `modalities=['text']`에서도 맥락에 맞다고 판단하면 추가 문장을 생성한다.
이를 방지하기 위해 번역 단계를 Chat API (gpt-4o-mini, temperature=0)로 분리하여 입력 텍스트만 정확히 번역한다.

### 아키텍처

```
Twilio Audio (g711_ulaw)
    │
    ▼
3-Stage Filtering (Echo Gate → Energy Gate → Local VAD)
    │
    ▼
OpenAI Realtime API (modalities=['text'])
    ├─ STT: input_audio_transcription.completed → stt_text
    ├─ Blocklist/Noise 필터링
    ├─ context_prune_keep=0 (Realtime 컨텍스트 아이템 전부 삭제 → 자체 번역 생성 방지)
    └─ _stt_text 저장 + _stt_ready_event.set()
           ↓
Chat API (gpt-4o-mini) via ChatTranslator
    ├─ System: "Translate only, no extra words"
    ├─ Context: 2턴 슬라이딩 윈도우 (번역 일관성)
    ├─ temperature=0, max_tokens=300
    └─ Output: translated_text → caption
```

### V2V와의 차이

| 항목 | V2V Session B | T2V/Agent Session B |
|------|--------------|---------------------|
| STT | Realtime (Whisper-1) | Realtime (Whisper-1) |
| 번역 | **Realtime API** (gpt-4o-realtime) | **Chat API** (gpt-4o-mini) |
| 오디오 출력 | TTS pcm16 24kHz | None (text only) |
| modalities | `['text', 'audio']` | `['text']` |
| 컨텍스트 윈도우 | 6턴 | 2턴 |
| context_prune_keep | 1 | **0** (전부 삭제) |
| Echo Window Max | 1.2s | 5.0s (cap 없음) |
| 비용 추적 | Realtime tokens만 | Realtime + Chat tokens |

### 핵심 코드 경로

```python
# session_b.py: STT 완료 시 Chat API 번역 트리거
async def _handle_stt_transcript(self, text: str):
    if self._chat_translator:
        self._stt_text = text
        self._stt_ready_event.set()  # _translate_via_chat_api() 대기 해제

# session_b.py: response.done 시 Chat API 번역 실행
async def _translate_via_chat_api(self):
    await self._stt_ready_event.wait()     # STT 완료 대기
    result = await self._chat_translator.translate(self._stt_text)
    # → caption + transcript 저장
```
...
| 파라미터 | 프로덕션 값 | 설명 |
|----------|------------|------|
| `session_b_use_chat_translation` | `True` | T2V/Agent에서 Chat API 번역 활성화 |
| `session_b_chat_translation_model` | `gpt-4o-mini` | Chat API 모델 |
| `session_b_chat_translation_timeout_ms` | `3000` | 요청 타임아웃 (ms) |

### 비용 추적

T2V/Agent에서 Chat API 토큰은 `CostTokens.chat_input` / `CostTokens.chat_output` 필드로 별도 추적된다.
Realtime API 토큰과 합산하여 총 비용을 산출한다.
...
| Min speech duration | 250ms | config.py:92 |
| Peak RMS quality filter | 300 RMS | config.py:109 |
| Echo energy threshold | 500 RMS | config.py:108 |
...
| Chat API 번역 활성화 | True | config.py:120 (T2V/Agent Session B) |
| Chat API 모델 | gpt-4o-mini | config.py:121 |
| Chat API 타임아웃 | 3000ms | config.py:122 |
| context_prune_keep (V2V) | 1 | text_to_voice.py:125 |
| context_prune_keep (T2V/Agent) | 0 | text_to_voice.py:137 |
